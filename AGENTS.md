...
- **On the unit commit:** mark **Status** in the plan (same commit if the plan file is in the tree; otherwise a follow-up). If a CE explainer for that unit exists under `.compound-engineering/artifacts/explainers/`, include it in the same commit (and link it from the plan unit). Do not leave the explainer for a follow-up. Then push only when the CTO asks. The unit commit must end with `Closes #n`. GitHub closes that issue only when the commit reaches `main`, not on a local commit. Do not `gh issue close` unless they ask; the keyword is the close path.
- **After a unit CE code review** (`ce-code-review` / multi-agent review of that unit’s diff — not named tests alone, not a casual glance): offer an English summary comment on that unit’s GitHub issue (verdict, actionable findings or none, residual risks / testing gaps worth keeping). Do not post the comment until the CTO says yes.

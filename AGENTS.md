...
- **Before starting a unit:** if that unit has no GitHub issue, say so and wait. Do not open one unasked. This cut: U1–U5 are sub-issues of #1; U6 and U7 are not on GitHub yet (see the plan’s Issues index).
- **After the CTO accepts a unit:** mark **Status** in the plan first, then remind them to close the matching issue. Do not close it unless they ask. Do not put `Closes #n` / `Fixes #n` in commits — that would close the ticket before the CTO accepts.
...
- **Commit messages** name the plan unit and the zone, then the why: `feat(api): U5 list cases and events for console poll`. Zones: `api`, `web`, `sim`, `docs`, `dev` (Compose, Makefile, `.devcontainer`, run-path glue — not product code). Do not commit unless the CTO asks.
